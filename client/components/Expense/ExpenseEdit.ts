...
@Component({
    selector: 'NewExpenseEdit',
    template: require('./ExpenseEdit.html'),
    styles: [`
    input.ng-dirty.ng-invalid { border: solid red 2px; }
    input.ng-dirty.ng-valid { border: solid green 2px; }`]

})
export class ExpenseEditComponent implements OnInit, OnDestroy, ExpenseObserver {
    ...
    constructor(private router: Router, private repository: ExpenseService, private activatedRoute: ActivatedRoute) {
